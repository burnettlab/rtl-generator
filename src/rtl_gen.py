...
def run_generator(rtl_name: str, pretty_rtl_name: str, cli_args: argparse.Namespace, mod_str: str, **kwargs) -> str:
    """
    ...
    return rtl_generator(rtl_name, pretty_rtl_name, cli_args, mod_vars, **kwargs) + "\n"

@dataclass
class setup:
    """
    Setup the RTL generation environment.

    Script entry
    """
    cli_args: argparse.Namespace

    def __call__(self, rtl_name: str, pretty_rtl_name: str, proj_path: str | Path) -> None:
        if os.path.exists(str(Path(proj_path, "options.yml"))):
            print("RTL Generator environment already exists")
            return
        
        print(f"Setting up RTL Generator environment for project: {pretty_rtl_name}")
        ...
        with open(Path(Path(__file__).parent, "top_level.py")) as f:
            top_level_py = f.read()
        top_level_py = fill_in_template(top_level_py, None, vars(), regex_str=r"#{\((\w+?)\)}")
        with open(Path(proj_path, f"gen_{rtl_name}.py"), "w") as f:
            f.write(top_level_py)
        
        update(self.cli_args)(rtl_name, pretty_rtl_name, proj_path)

        print(f"\nFinished setting up RTL Generator environment for project: {pretty_rtl_name}")
        print('-' * os.get_terminal_size().columns)

    @staticmethod
    def add_args(rtl_name: str, pretty_rtl_name: str, proj_path: str | Path, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        return parser
...
class update:
    ...
    def __call__(self, rtl_name: str, pretty_rtl_name: str, proj_path: str | Path) -> None:
        print(f"Updating generators in RTL heirarchy for: {pretty_rtl_name}")
        ...
        for submod in get_subdirs(proj_path):
            ...
            if not gen_path.exists():
                with open(Path(Path(__file__).parent, "sub_level.py")) as f:
                    sub_level_py = f.read()
                sub_level_py = fill_in_template(sub_level_py, None, vars(), regex_str=r"#{\((\w+?)\)}")
                with open(gen_path, "w") as f:
                    f.write(sub_level_py)

            self(submod_name, pretty_submod_name, submod)

            os.chdir(proj_path)

        print(f"\nFinished updating generators in RTL heirarchy for: {pretty_rtl_name}")
        print('-' * os.get_terminal_size().columns)
...
class generate:
    ...
    def __call__(self, rtl_name: str, pretty_rtl_name: str, proj_path: str | Path) -> None:
        print(f"Generating RTL for project: {pretty_rtl_name}")

        generated_rtl = run_generator(rtl_name, pretty_rtl_name, self.cli_args, f"gen_{rtl_name}")
        ...
        while available_submods:
            ...
            submod_rtls[name] = run_generator(name, pretty_name, self.cli_args, f"{name}.gen_{name}")

            available_submods.extend(get_subdirs(submod_path))
            os.chdir(proj_path)

        if self.cli_args.replace_includes:
            submod_rtls = {rtl_name: replace_includes(pretty_rtl_name, generated_rtl, submod_rtls)}
        else:
        ...
        print(f"\nFinished generating RTL for project: {pretty_rtl_name}")
        print('-' * os.get_terminal_size().columns)

    @staticmethod
    def add_args(rtl_name: str, pretty_rtl_name: str, proj_path: str | Path, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        return add_args(rtl_name, pretty_rtl_name, proj_path, parser)
...
def main() -> None:
    ...
    # Create CLI
    parser = argparse.ArgumentParser(description="RTL Generator")
    ...
    for f in filter(lambda f: callable(f) and f.__doc__ and f.__doc__.strip().splitlines()[-1].endswith("Script entry"), globals().values()):
        ...
        subparser = subparsers.add_parser(command_name, help=help_str)
        f.add_args(rtl_name, pretty_rtl_name, proj_path, subparser)
        subparser.set_defaults(func=f)

    # Parse arguments and run function
    args = parser.parse_args()
    func = args.func(args)
    func(rtl_name, pretty_rtl_name, proj_path)
